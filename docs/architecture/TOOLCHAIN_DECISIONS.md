...
| Alternate Python GUI framework | local project decision | The prior alternate frontend preview lane was removed from this checkout. | Do not add a new GUI framework during core download-engine, controller, or build-script refactors; extract toolkit-neutral seams first and validate the existing FireDM GUI path. | observed |
| wxPython | https://wxpython.org/pages/overview/ | wxPython is cross-platform and uses native widgets through wxWidgets. | Deferred; not selected while core/view seams remain coupled. | verified |
| Dear PyGui | https://dearpygui.readthedocs.io/ | GPU-accelerated cross-platform GUI toolkit for Python. | Deferred; not selected while core/view seams remain coupled. | verified |
