...
def test_verify_extractor_default_script_passes(tmp_path):
    ...
    proof_dir = tmp_path / "extractor"
    r = _run(["scripts/verify_extractor_default.py", "--output-dir", str(proof_dir)], timeout=120)
    assert r.returncode == 0, f"stdout={r.stdout}\nstderr={r.stderr}"
    proof = proof_dir / "default_selection_proof.json"
...
def test_smoke_video_pipeline_script_passes(tmp_path):
    r = _run(["scripts/smoke_video_pipeline.py", "--output-dir", str(tmp_path / "smoke")], timeout=60)
